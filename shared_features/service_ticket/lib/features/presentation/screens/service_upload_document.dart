...
class ServiceUploadDocument extends StatefulWidget {
  final String imagePath;

  const ServiceUploadDocument({super.key, required this.imagePath});

  @override
  State<ServiceUploadDocument> createState() => _ServiceUploadDocumentState();
}

class _ServiceUploadDocumentState extends State<ServiceUploadDocument> {
  XFile? _pickedFile;
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        appBar: customAppbar(
          context: context,
          title: getString(lblUploadDocument),
          onPressed: () {
          ...
          listener: (context, state) {
            if (state is GetPresetUriResponseSuccessState) {
              if (state.response.code == AppConst.codeSuccess) {
                context.read<ServiceRequestCubit>().uploadDocument(
        ...
        floatingActionButton: Padding(
          ...
          child: CustomOutlinedButton(
            ...
            onPressed: () {
              ...
              GetPresetUriRequest(
                  fileName: filename,
                  useCase: SRConst.uploadDocument);
              context.read<ServiceRequestCubit>().getPresetUri(request,
                  operation: SRConst.uploadDocument);
            },
          ),
        ),
      ),
