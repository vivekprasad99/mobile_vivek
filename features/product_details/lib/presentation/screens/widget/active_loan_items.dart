...
class ActiveLoansItem extends StatefulWidget {
  ActiveLoansItem(
    this.data,
    this.tabType, {
    super.key,
  ...
  @override
  State<ActiveLoansItem> createState() => _ActiveLoansItemState();
}

class _ActiveLoansItemState extends State<ActiveLoansItem> {
  int loanLength = 2;
            ...
            children: [
              ...
              Column(
                ...
                children: [
                  ...
                  SizedBox(height: 2.v),
                  Text( widget.tabType.equalsIgnoreCase('completed')
                      ?dueDate:
                      formatCurrency(getTotalPayableAmount(datum)),
                      style: Theme.of(context).textTheme.labelMedium),
                  SizedBox(height: 17.v),
                  Text(
                    widget.tabType.equalsIgnoreCase('completed')
                  ...
                  SizedBox(height: 4.v),
                  Text(widget.tabType.equalsIgnoreCase('completed')
                      ?getString(lblAvailableProductDetail):dueDate, style: Theme.of(context).textTheme.labelMedium),
                ],
              )
            ],
          ),
      ...
      child: ElevatedButton(
          ...
          onPressed: () {
            if (isActiveMandate(datum.mandateStatus ?? "")) {
              ...
              if(!isMandateDisable(datum.mandateStatus ?? "", datum.sourceSystem ?? "")) {
          ...
          child: Opacity(opacity: isMandateDisable(datum.mandateStatus ?? "", datum.sourceSystem ?? "") ? 0.4 : 1.0, child: Text(
              isActiveMandate(datum.mandateStatus ?? "")
                  ? getString(lblAvailTopUp)
                  : getString(lblCreateMandate),
