...
import 'package:product_details/presentation/screens/widget/completed_loan_items.dart';
...
class CompletedLoanListScreen extends StatefulWidget {
  CompletedLoanListScreen(this.tabType, {super.key, this.data});

  String tabType;
  ServicesNavigationRequest? data;

  @override
  CompletedLoanListScreenState createState() => CompletedLoanListScreenState();
}

class CompletedLoanListScreenState extends State<CompletedLoanListScreen>
    with AutomaticKeepAliveClientMixin<CompletedLoanListScreen> {
  bool _isAppBarEnabled = false;

  @override
  void initState() {
    if (widget.tabType == 'completed' && isCustomer()) {
  ...
  List<ActiveLoanData> get completedLoans => loanLists
      .where((loan) =>
    ...
    if (completedLoans.isNotEmpty) {
      return Column(
        ...
        children: [
          ...
          CompletedLoansItem(completedLoans, widget.tabType),
