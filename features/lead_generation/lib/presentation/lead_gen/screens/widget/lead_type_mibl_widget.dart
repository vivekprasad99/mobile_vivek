...
class LeadTypeMiblWidget extends StatefulWidget {
  final String leadType;
  const LeadTypeMiblWidget({
    super.key,
    required this.leadType,
  });

  @override
  State<LeadTypeMiblWidget> createState() => _LeadTypeMiblWidgetState();
}

class _LeadTypeMiblWidgetState extends State<LeadTypeMiblWidget> {
  final GlobalKey<FormState> _formKey = GlobalKey<FormState>();
  final TextEditingController _fullNameController =
      TextEditingController(text: getUserName());
  final TextEditingController _pinCodeVInsuranceController =
  ...
  LeadProductsData? _leadProductsData;
  @override
  void initState() {
    context.read<LeadGenerationCubit>().getProductsVal();
    super.initState();
  }
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // SizedBox(height: 8.v),
                  // _buildVehicleRegistrationNumber(),
                  SizedBox(height: 20.v),
                  BlocBuilder<LeadGenerationCubit, LeadGenerationState>(
                    buildWhen: (previous, current) {
                      return current is FetchInsuranceProductsSuccessState;
                    },
                    builder: (context, state) {
                      ...
                      if (state is FetchInsuranceProductsSuccessState) {
                        List<LeadProductsData>? products = state.response.data;
                        return CustomDropDownButton<LeadProductsData>(
                          isExpanded: true,
                          hintText: getString(lblInsuranceType),
                          items: products!
                              .where((element) => element.lob!
                                  .equalsIgnoreCase(widget.leadType))
                              .map((e) => DropdownMenuEntry<LeadProductsData>(
                                  label: e.productName.toString(), value: e))
                              .toList(),
                          onChanged: (value) {
                            // context
                            ...
                            setState(() {
                              _leadProductsData = value;
                            });
                          },
                          value: _leadProductsData,
                        );
                      }
                      return Container();
                    },
  ...
  Widget _buildStateCityWidget() {
    return BlocBuilder<LeadGenerationCubit, LeadGenerationState>(
      ...
      return Padding(
        ...
        child: Text(
          _pinCodeVInsuranceController.text.isEmpty || _stateCity == null
  ...
  Widget _buildFirstName() {
    return CustomFloatingTextField(
      onChange: (value) {
        value.isEmpty || _pinCodeVInsuranceController.text.length != 6
  ...
  Widget _buildPinCode() {
    return CustomFloatingTextField(
      ...
      maxLength: 6,
      controller: _pinCodeVInsuranceController,
    ...
    return BlocBuilder<LeadGenerationCubit, LeadGenerationState>(
      ...
      builder: (context, state) {
        ...
        return CustomElevatedButton(
            ...
            onPressed: () {
              if (_pinCodeVInsuranceController.text.isNotEmpty &&
                  _fullNameController.text.isNotEmpty) {
                BlocProvider.of<LeadGenerationCubit>(context)
                    .submitActionPressed(
                        leadGenerationRequest: LeadGenerationRequest(
                  name: _fullNameController.text,
                  leadType: LeadType.mibl.value,
                  pinCode: _pinCodeVInsuranceController.text,
                  mobileNumber: _phoneNumberController.text,
                  customerId: getUCIC(),
                  product: _leadProductsData?.productValue,
